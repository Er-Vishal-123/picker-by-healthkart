...
const PerformanceDashboard = ({ pickers }: PerformanceDashboardProps) => {
  const topPerformer = pickers.reduce((prev, current) => 
    prev.efficiency > current.efficiency ? prev : current
  );

  const fastestPicker = pickers.reduce((prev, current) => 
    prev.timePerItem < current.timePerItem ? prev : current
  );

  const mostAccurate = pickers.reduce((prev, current) => 
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-yellow-200">{topPerformer.efficiency}% efficiency</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xl font-bold">{fastestPicker.name}</p>
                <p className="text-sm text-blue-200">{fastestPicker.timePerItem}s per item</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xl font-bold">{mostAccurate.name}</p>
                <p className="text-sm text-green-200">{mostAccurate.pickAccuracy}% accuracy</p>
              </div>
